<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Analisis de Cotizacion</title>

    <style>
* {
    margin: 0;
    padding: 0;
    box-sizing: border-box;
    }
    html, body {
    width: 95%;
    height: 100%;
    margin-top: 15px; 
    }
    img
    {
    width: 150px;
    height: 150px;
    }
    .titulo{
    font-size: 9px;
    margin-left: 15px; 
    font-family: Arial, sans-serif;
    
    }
   .titulo2{
    font-size: 13px;
    margin-left: 15px; 
    font-family: Arial, sans-serif;
    margin-top: 20px; 
    /*background-color:#0380FF;*/ 

    }
    .subtitulo{
    font-size: 19px; 
    margin:19px;
    }
    hr {
    height: 20px;
    width: 100%;
    /*background-color:#0380FF;*/
    background-color: #000000;
    }
    td{
    padding-left:15px;
    }
    .resumen1{
    margin-left:10px;
    }
    .resumen1 th{
    font-size: 13px;
    }
    .resumen1 td{
    font-size: 13px;
    }
    .resumen{
    margin-left:10px; 
    }
    .resumen th{
    font-size: 10px;
    }
    .resumen td{
    font-size: 10px;
    }
    P{
    font-size: 15px;  
    margin-left: 10px; 
    }
    .footer {
    text-align: center;
    justify-content: center;
    margin:auto;
    /*background-color: #F1C40F;*/
    position: fixed;
    width: 95%;
    height: 100px;
    bottom:0;
    margin-bottom:50px;
    margin-left:10px;
    }
    .lateral{
    height: 50px; 
    /*background-color: blue; */
    }

   .firma{
    font-size: 10px;
  }
  .pie{
    font-size: 8px;
    margin-left: 5px;  
    
  }
  .contenido{
    text-align: center;
    justify-content: center;
    
    margin-left:20px; 
  }

</style>
<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
</head>
<body>
<div class="container-fluid">
       <header>
            <section class="content container-fluid">
                <div class="row text-center justify-content">
                    <div class="col-md-12">  
                            <div class="float-left">
                                <img src="data:image/png;base64,">
                            </div> 
                    </div>
                    <div class="col-md-12">  
                            <div class="text-center justify-content">
                                    <h2 class="titulo2 "  >REPÚBLICA BOLIVARIANA DE VENEZUELA
                                    PROANZOÁTEGUI </h2>
                                    <h2 class="titulo2  " >DIRECCIÓN DE ADMINISTRACIÓN
                                    DEPARTAMENTO DE COMPRAS</h2>
                                    <h3 class="subtitulo">HOJA DE ANÁLISIS</h3>
                            
                            </div> 
                    </div>
                </div>
            </section>   
        </header>
        <main>
            <table class="table table-bordered resumen table-sm">
                <thead>
                </thead>
                <tbody>
                    <tr>
                        <td rowspan="2">UNIDAD SOLICITANTE: {{ $analisi->unidadadministrativa->unidadejecutora }}</td>
                        <td class="text-center justify-content">FECHA DE ANALISIS</td>

                        <td colspan="2" class="text-center justify-content">REQUISICIÓN</td>
                    
                    </tr>
                   
                    <tr>
                    <td class="text-center justify-content"> {{ $analisi->created_at->toDateString() }}</td>  
                    <td class="text-center justify-content">FECHA: {{ $analisi->requisicione->created_at->toDateString()  }}</td>
                    <td class="text-center justify-content">N° 

    @if($analisi->requisicione->correlativo < 10)
            {{  '00' . $analisi->requisicione->correlativo }}

    @elseif($analisi->requisicione->correlativo >= 10 && $analisi->requisicione->correlativo < 100)
            {{ '0' . $analisi->requisicione->correlativo }}
    @elseif($analisi->requisicione->correlativo >= 1000)
            {{  $analisi->requisicione->correlativo }}

    @endif

                    </td>   
                    </tr>
                </tbody>
            </table>
            <table class="table table-bordered resumen table-sm">
                <thead>
                </thead>
                <tbody>

                

                <tr class="table-secondary">
                        <td rowspan="3" class=" text-center justify-content">ITEM</td>
                        <td rowspan="3">DESCRIPCION DEL BIEN</td>
                        <td rowspan="3">CANTIDAD</td>

                        <td colspan="2">PROVEEDOR A</td>
                        <td  colspan="2">PROVEEDOR B</td>
                        <td  colspan="2">PROVEEDOR C</td>

                        <td rowspan="3">OBSERVACIÓN</td>
                    
                    </tr>
                   
                    <tr>
                    <td colspan="2">{{ $beneficiario->nombre }}</td>  
                    <td colspan="2">{{ $beneficiario_b->nombre }}</td>
                    <td colspan="2">{{ $beneficiario_c->nombre }}</td>   
                    </tr>
                    <tr>
                        <td class="table-secondary" >PRECIO UNIT.</td>
                        <td class="table-secondary">MONTO BsF.</td>
                        <td class="table-secondary">PRECIO UNIT.</td>
                        <td class="table-secondary">MONTO BsF.</td>
                        <td class="table-secondary">PRECIO UNIT.</td>
                        <td class="table-secondary">MONTO BsF.</td>
                    </tr>

                    <?php $i=0; ?>

                    {{-- Se coloca el foreach, para colocar los detalles del analisis --}}
                  
                    @foreach($detallesanalisis as $rows)
                    
                    <tr>
                        <td>{{ ++$i }}</td>
                        <td>
                            {{ $rows->bo->descripcion }}
                        </td>
                        <td class="text-right justify-content">{{ number_format($rows->cantidad,2,',','.') }}</td>
                        <td class="text-right justify-content">{{ number_format($rows->precio,2,',','.') }}</td>
                        <td class="text-right justify-content">{{ number_format($rows->subtotal,2,',','.') }}</td>
                        <td class="text-right justify-content">{{ number_format($rows->preciob,2,',','.') }}</td>
                        <td class="text-right justify-content">{{ number_format($rows->subtotalb,2,',','.') }}</td>
                        <td class="text-right justify-content">{{ number_format($rows->precioc,2,',','.') }}</td>
                        <td class="text-right justify-content">{{ number_format($rows->subtotalc,2,',','.') }}</td>
                        <td></td>
                    </tr>

                    @endforeach

                    
                    

                    
                    <tr>
                    <td colspan="4">Total</td>
                    <td class="text-right justify-content">{{ number_format($total,2,',','.') }}</td>
                    <td></td>
                    <td class="text-right justify-content">{{ number_format($total_b,2,',','.') }}</td>
                    <td></td>
                    <td class="text-right justify-content">{{ number_format($total_c,2,',','.') }}</td>
                    <td></td>
                   

                    </tr>
                </tbody>
            </table>

            <table class="table table-bordered resumen table-sm">
                <thead class="table-secondary">
                    <tr>
                        <th colspan="3" class="text-center justify-content">PROVEEDOR BENEFICIADO</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>NOMBRE: {{ $beneficiario->nombre }}</td>
                        <td>RIF: {{ $beneficiario->caracterbeneficiario .' - ' . $beneficiario->rif }}</td>
                        <td>TELEFONOS: {{ $beneficiario->telefono }}</td>
                    </tr>
                    <tr>
                        <td colspan="3">DIRECCIÓN: {{ $beneficiario->direccion }}</td>
                    </tr>
                </tbody>
           </table>
           <table class="table table-bordered resumen table-sm">
                <thead class="table-secondary">
                    <tr>
                        <th colspan="10" class="text-center justify-content">CRITERIOS DE SELECCIÓN</th>
                    </tr>
                </thead>
                <tbody>
                    <tr >
                        <td colspan="10">
                            {{ $analisi->criterio->nombre }}
                        </td>
                                             
                    </tr>
                   
                    <tr>
                        <td colspan="10" >OBSERVACION:  {{ $analisi->observacion }} </tr>
                </tbody>
           </table>
        </main>	
        <br>  <br>
        <footer class="footer">
              <table class="table table-bordered">
                <thead >
                  <tr>
                    <th class="firma" >ELABORADO POR:</th>
                    <th  class="firma"  >CONFIRMADO POR:</th>
                   
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td class="lateral"></td>
                    <td></td>
                  </tr>
                </tbody>
               
              </table>
              <div class="pie text-left justify-left">Elaborado por: {{ $analisi->usuario->name }}</div>
            </footer>	                                 
  </body>
</html>
