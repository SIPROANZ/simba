...
//inicio else precompromiso
elseif($tipo == 'PCOMPROM'){
    //Obtener los datos del precompromiso de existir
    $rs_precompromiso = Precompromiso::find($ruta_id);
    if($rs_precompromiso){ //validar que existe el precompromiso
        $id = $ruta_id;
        //Datos para el precompromiso
        $estado = $rs_precompromiso->status;
        if($estado== 'AP'){
            $estado ="APROBADO";
        }elseif ($estado== 'EP') {
            $estado ="EN PROCESO";
        ...
        $correlativo=$rs_precompromiso->documento;


        $creado =$rs_precompromiso->created_at;
        $modificado=$rs_precompromiso->updated_at;

    $concepto = substr($rs_precompromiso->concepto, 0, 300);

    $nombre_tipo = "Precompromiso";

    $rs_user = User::find($rs_precompromiso->usuario_id);
    if($rs_user){
        $elaborado =$rs_user->name;
    }

    //Area para colocar el compromiso, el causado y el pagado
     //Inicio de compromiso y causado y pagado
        //Si existe la compra verificar que exista el compromiso
        $rs_compromiso = Compromiso::where('precompromiso_id', $id)->first();
        if($rs_compromiso){  //
            $compromiso = $rs_compromiso->id;
        ...
        //fin de inicio compromiso, causado y pagado
    //Fin area para colocar compromiso, causado y pagado

    } //Fin de validar que el precompromiso existe

}
//fin de else precompriso
//Inicio de else compromiso
if($tipo == 'COMPROM'){
 //Verificar que exista el compromiso
 
 $rs_compromiso = Compromiso::where('id', $ruta_id)->first();
        if($rs_compromiso){  //
            $compromiso = $rs_compromiso->id;
            ...
            //Inicio de validar si es un precompromiso, una ayuda, o una compra
            //validar que sea una precompromiso
             //Inicio precompromiso, ayuda, compra
            if($rs_compromiso->precompromiso_id){
                $id = $rs_compromiso->precompromiso_id;

                $rs_precompromiso = Precompromiso::find($id);
                if($rs_precompromiso){ //validar que existe el precompromiso
                   // $id = $ruta_id;
                    //Datos para el precompromiso
                    $estado = $rs_precompromiso->status;
            ...
            //fin de validar precompromiso

            //Inicio validar ayuda
           
            if($rs_compromiso->ayuda_id){

                $id = $rs_compromiso->ayuda_id;
                $rs_ayuda = Ayudassociale::find($id);
                if($rs_ayuda){
            ...
            if($rs_compromiso->compra_id){
                $id = $rs_compromiso->compra_id;

                //obtener si aparece el registro de compra
                $rs_compra = Compra::find($id);
                if($rs_compra){
                     //Obtener el id analisis
            $rs_analisis = Analisi::find($rs_compra->analisis_id);
            if($rs_analisis){
    ...
    if($rs_user){
        $elaborado =$rs_user->name;
            ...
            //Fin de validar si es un precompromiso, una ayuda, o una compra

        }
        //fin de inicio compromiso, causado y pagado
    //Fin area para colocar compromiso, causado y pagado
}
//Fin de else compromiso

//inicio else ODPAGO
elseif($tipo == 'ODPAGO'){
$rs_causado = Ordenpago::find($ruta_id);
if($rs_causado){ //validacion causado
//si existe el compromiso verificar que exista el causado
    ...
    //Inicio Compromiso
    $rs_compromiso = Compromiso::where('id', $rs_causado->compromiso_id)->first();
        if($rs_compromiso){  //
            $compromiso = $rs_compromiso->id;
            ...
            $monto_compromiso = number_format($rs_compromiso->montocompromiso, 2, ',', '.');
                ...
                //Inicio obtener el tipo de compromiso
                 //Inicio precompromiso, ayuda, compra
            if($rs_compromiso->precompromiso_id){
                $id = $rs_compromiso->precompromiso_id;
    ...
    if($rs_user){
        $elaborado =$rs_user->name;
                ...
                //Fin de obtener el tipo de compromiso
...
} //Fin validacion causado
}
//FIN ELSE ODPAGO

//Inicio else pagado
if($tipo == 'PAG'){
$rs_pagado = Pagado::find($ruta_id);
//Inicio validacion pagado
if($rs_pagado){
    $pagado = $rs_pagado->id;
...
$monto_pagado = number_format($rs_pagado->montopagado, 2, ',', '.');
$monto_restante = number_format(($rs_pagado->montoordenpago - $rs_pagado->montopagado), 2, ',', '.');
//iNICIO PARA OBTENER EL CAUSADO
$rs_causado = Ordenpago::find($rs_pagado->ordenpago_id);
if($rs_causado){ //validacion causado
//si existe el compromiso verificar que exista el causado
    ...
    //inicio para obtener el compromiso
      //Inicio Compromiso
      $rs_compromiso = Compromiso::where('id', $rs_causado->compromiso_id)->first();
      if($rs_compromiso){  //
          $compromiso = $rs_compromiso->id;
  ...
  //Fin de compromiso
    //Fin de obtener el compromiso

}
//fIN DE OBTNER EL CAUSADO

}
//Fin validacion pagado
...
//immprimir la ruta obtenida, imprimirlo en un archivo PDF
...
//Fin else pagado
   ...
   $pdf = PDF::loadView('rutapdf', ['ruta_id'=>$ruta_id,'tipo'=>$tipo,'datos'=>$datos]);
